...
| `UsersController` | User discovery, presence-related endpoints, `GET /api/users/recent-activity` (auth) for recent players / DMs / joined rooms |
...
| `MatchmakingController` | Quick match queue: `POST/DELETE /api/matchmaking/queue`, `GET /api/matchmaking/queue/status` (auth) — pairs players into a new “Quick Match” room or surfaces fallback room picks |
...
- **Routing and guards:** `client/src/app/router.tsx` — `ProtectedRoute` and `RequireProfile` protect most app routes; `/subscriptions` may omit full profile requirement; `/moderation` uses `RequireRole` for moderator/admin. **`/compare`** (`comparePlayers.tsx`) compares two players side by side (profiles + Elo) with shareable `?left=&right=` query params.
...
| [docs/NoobGg-Musteri-Durum-Raporu.md](docs/NoobGg-Musteri-Durum-Raporu.md) | Customer-facing status report (Turkish): pages, features, gaps, roadmap |
